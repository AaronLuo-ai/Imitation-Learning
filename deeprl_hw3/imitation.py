...
def generate_expert_training_data(expert, env, num_episodes, render = False):
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            states.append(state)
            # print("state_list shape: ",states.shape)
            action_prob = expert.predict(state.reshape(1, -1))
            print("action_prob shape: ", action_prob.shape)
            print("state.reshape(1, -1) shape: ", state.reshape(1, -1).shape)
            action = np.argmax(action_prob, axis=1)[0]  # Assuming the expert model outputs probabilities for each action
            print("action: ",action)
...
def test_cloned_policy(env, cloned_policy, num_episodes, render=True):
